    ...
    if (File.Exists(filename))
    {
        ...
        using (StreamReader reader = new StreamReader(filename))
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                {
                    continue; // Invalid goal data format, skip this line
                }

                string[] details = goalData[1].Split(',');
                if (details.Length < 2)
                {
                    continue; // Invalid goal details format, skip this line
                }
                ...
                switch (goalType)
                {
                    ...
                    case nameof(ChecklistGoal):
                        if (details.Length != 5)
                        {
                            continue; // Invalid checklist goal details format, skip this line
                        }
                        int desiredAmount;
                        ...
                        break;
                    default:
                        continue; // Unknown goal type, skip this line
                }
